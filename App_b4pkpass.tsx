              ...
              <ScrollView style={styles.modalBody}>
                ...
                {Platform.OS === 'ios' ? (
                  <AddPassButton
                    ...
                    onPress={async () => {
                      try {
                        // Fetch the pkpass file as binary
                        const createPassResponse = await axios.post(
                          'http://127.0.0.1:5001/test-credo-project/us-central1/pass',
                          {
                            thumbnailUrl:
                              'https://cdn.fliki.ai/image/page/660ba680adaa44a37532fd97/666310fc647d79dc96fd21a3.jpg',
                            primary: {
                              value: 'Blockchain Club',
                              label: 'Club Name:',
                            },
                            secondary: [
                              {value: 'Sophia', label: 'First Name'},
                              {value: 'Founder', label: 'Position'},
                            ],
                            auxiliary: [
                              {value: 'May 25, 2025', label: 'Expiration Date'},
                              {value: '(917)-870-7296', label: 'Phone Number'},
                            ],
                            backgroundColor: '#FFFFFF',
                            textColor: '#000000',
                            logoText: 'Blockchain Club',
                            description: 'Blockchain Club Membership Pass',
                          },
                        ...
                        const base64Pass = Buffer.from(
                          createPassResponse.data,
                          'binary',
                        ).toString('base64');
                        await PassKit.addPass(base64Pass);
                      } catch (error: any) {
                        console.error(
                          'Error creating or downloading pass:',
                          error,
                        );
                        Alert.alert(
                          'Error',
                          error.message ||
                            'Failed to create or download pass. Please try again.',
                        );
                      }
                    }}
                  />
